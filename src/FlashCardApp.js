...
const FlashCardApp = () => {
  ...
  // Combine both lists
  const allWords = [...vocabularyWords, ...extendedVocabularyWords];
  ...
  const handlePrevCard = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + allWords.length) % allWords.length);
    setShowDefinition(false);
  };
        ...
        <div className="w-full max-w-lg">
          <div className="bg-white rounded-lg shadow-lg overflow-hidden border-2 border-gray-300 flex flex-col min-h-64">
            ...
            <div className="flex-grow flex flex-col justify-center items-center p-6 text-center">
              <h2 className="text-3xl font-bold mb-4">{allWords[currentIndex].word}</h2>
              <p className="text-gray-500 italic mb-2">{allWords[currentIndex].partOfSpeech}</p>
              
              {showDefinition && (
                <div className="mt-2 text-left w-full">
                  <p className="font-semibold">Definition:</p>
                  <p className="mb-2">{allWords[currentIndex].definition}</p>
                  <p className="font-semibold">Example:</p>
                  <p className="italic">{allWords[currentIndex].example}</p>
                </div>
              )}
            </div>
          </div>
